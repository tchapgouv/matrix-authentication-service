...
pub async fn async_main(compiled_config: Option<CompiledConfig>) -> anyhow::Result<ExitCode> {
    ...
    // error or not
    let res = try_main(compiled_config).await;
...
async fn try_main(compiled_config: Option<CompiledConfig>) -> anyhow::Result<ExitCode> {
    ...
    opts.run(&figment, compiled_config).await
}
