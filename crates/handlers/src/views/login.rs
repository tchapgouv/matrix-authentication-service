...
pub(crate) async fn post(
    ...
    if site_config.login_with_email_allowed && form.username.contains('@') {
        ...
        let email_check =
            tchap::is_email_allowed(email, homeserver.homeserver(), &tchap_config).await;
        ...
        }) = &email_check
        {
            ...
            let form_state = form_state.with_error_on_form(FormError::Policy {
                code: None,
                message: format!(
                       "Adresse mail {email} associée au serveur:{correct_server_name} hors vous êtes sur le serveur:{wrong_server_name}. Veuillez contactez le support: support@tchap.beta.gouv.fr",
                ),
            });
            return render(
                locale,
...
mod test {
    ...
    //:tchap:
    /// Start a mock identity server where `wrong_server@example.com` is
    /// mapped to a different homeserver, and point the test state's tchap
    /// config at it. The returned server must be kept alive for the duration
    /// of the test.
    async fn mock_wrong_server_identity(state: &mut TestState) -> MockServer {
    ...
    async fn test_password_login_wrong_server_email(pool: PgPool) {
        ...
        let _identity_server = mock_wrong_server_identity(&mut state).await;
        let cookies = CookieHelper::new();
