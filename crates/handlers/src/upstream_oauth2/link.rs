                    ...
                    Some(localpart) => {
                        ...
                        if maybe_existing_user.is_none(){
                            ...
                            if let Ok(Some(email)) = maybe_email{
                                maybe_existing_user =  search_user_by_email(&mut repo, &email).await?;
                            }
                        }
                         //:tchap: end
...
pub(crate) async fn post(
    ...
    let session = match (maybe_user_session, link.user_id, form) {
        ...
        (None, None, FormData::Link) => {
            ...
            let template_username = provider
                .claims_imports
                .localpart
                .template
                .as_deref()
            ...
            let username = render_attribute_template(&env, template_username, &context, true)?;

            //:tchap: 
            //search by email also

            let template_email = provider
                .claims_imports
                .email
                .template
                .as_deref()
            ...
            let mut maybe_user = repo.user().find_by_username(&username.unwrap()).await?;
...
async fn search_user_by_email(
    ...
    tracing::info!("Email not found, Matching oidc identity by email using fallback rules:{}", email);
            ...
            if let (Some(match_pattern), Some(search_value)) = (
                ...
                if email.contains(match_pattern) {
                    ...
                    if let Some(transformed_user_email) = maybe_transformed_user_email {
                        ...
                        tracing::info!("User found with fallback rules {} : {}", match_pattern, search_value);
...
mod tests {
    ...
    use mas_data_model::{
        UpstreamOAuthAuthorizationSession, UpstreamOAuthLink, UpstreamOAuthProviderClaimsImports, UpstreamOAuthProviderImportPreference, UpstreamOAuthProviderOnConflict, UpstreamOAuthProviderTokenAuthMethod, User
    };
    ...
    async fn test_link_existing_account_by_email(pool: PgPool) {
        ...
        let claims_imports = UpstreamOAuthProviderClaimsImports {
            ...
            email: UpstreamOAuthProviderImportPreference {
                action: mas_data_model::UpstreamOAuthProviderImportAction::Require,
                template: Some(String::from("{{ user.email }}")),
                on_conflict: None,
            },
            ..UpstreamOAuthProviderClaimsImports::default()
        };
    ...
    async fn test_link_existing_account_by_email_with_fallback(pool: PgPool) {
        ...
        //check that the user email is not updated by oidc email
